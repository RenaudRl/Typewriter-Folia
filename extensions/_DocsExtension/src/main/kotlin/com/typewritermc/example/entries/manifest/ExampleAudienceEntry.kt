...
class TickableAudienceDisplay : AudienceDisplay(), TickableDisplay {
    ...
    override fun tick() {
        ...
        // If you need to interact with a player (modify inventory, effects, etc.)
        // you MUST use the player's entity dispatcher for Folia/Canvas compatibility
        players.forEach { player ->
            plugin.entityDispatcher(player).launch {
                // Safe to modify player here (inventory, effects, teleport, etc.)
            }
        }
    }
    // highlight-end
