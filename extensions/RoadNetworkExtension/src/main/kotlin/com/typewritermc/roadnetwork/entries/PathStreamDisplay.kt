...
abstract class PathStreamProducer(
    ...
    // GPS fetchers use entityDispatcher to ensure position callbacks that may access
    // world data (like Block.isPassable) are executed on the player's region thread for FOLIA
    protected val gps = PointToPointGPS(
        roadNetwork, 
        { withContext(plugin.entityDispatcher(player)) { startPosition(player) } }, 
        { withContext(plugin.entityDispatcher(player)) { endPosition(player) } }
    )
    ...
    /**
     * Gets the start and end positions for pathfinding.
     * 
     * Uses the player's entity dispatcher to ensure position callbacks that may access
     * world data (like block checks) are executed on the correct region thread for FOLIA.
     */
    suspend fun points(): Pair<Position, Position>? {
        // Execute position resolution on the player's region thread to ensure
        // any world data access (like Block.isPassable) is thread-safe for FOLIA
        val (start, end) = withContext(plugin.entityDispatcher(player)) {
            startPosition(player) to endPosition(player)
        }
    ...
    private suspend fun findPath(
        ...
        // Preload chunks needed for pathfinding
        // Double the margin to account for pathfinder exploration during A* search
        val margin = roadNetworkMaxDistance * 2
        val minX = kotlin.math.min(start.x, end.x) - margin
        val maxX = kotlin.math.max(start.x, end.x) + margin
        val minZ = kotlin.math.min(start.z, end.z) - margin
        val maxZ = kotlin.math.max(start.z, end.z) + margin
