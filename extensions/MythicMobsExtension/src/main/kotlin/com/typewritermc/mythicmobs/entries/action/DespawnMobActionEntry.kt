...
class DespawnMobActionEntry(
    ...
    override fun ActionTrigger.execute() {
        ...
        val mobsToRemove = MythicBukkit.inst().mobManager.activeMobs.filter { it.type == mob.get() }
        mobsToRemove.forEach { activeMob ->
            plugin.entityDispatcher(activeMob.entity.bukkitEntity).launch {
                activeMob.entity.remove()
                MythicBukkit.inst().mobManager.activeMobs.remove(activeMob)
            }
        }
    }
}