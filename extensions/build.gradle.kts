...
// Aggregated task to build all extensions for release
tasks.register("buildReleaseAll") {
    group = "build"
    description = "Builds all extensions for release"
    
    subprojects.filter { !it.name.startsWith("_") }.forEach { subproject ->
        dependsOn(":${subproject.name}:buildRelease")
    }
}
